            ...
            children: [
              for (final t in items)
                ListTile(
                  title: Text(t.title),
                  subtitle: Text(
                    t.kind == 'university_window'
                        ? 'University calendar · ${t.percent}%'
                        : 'Custom · ${t.percent}%',
                  ),
                  trailing: Text('${t.done}/${t.total}'),
                ),
            ],
          );
        },
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Name the tracker first.')),
      );
      return;
    }
    ...
    final result = await ref.read(trackersRepositoryProvider).createCustom(
          title: name,
