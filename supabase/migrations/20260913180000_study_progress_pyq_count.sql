-- Home subject cards counted lessons only. A PYQ can exist without a
-- lesson tag (topic_name / lesson_name optional), so the card said
-- "No lessons yet" while the subject list still showed the stem.
-- total_pyqs is distinct active questions on this subject's papers
-- for the student's university — including untagged rows.
