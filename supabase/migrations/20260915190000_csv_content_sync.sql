-- One-file CSV content importer. Both Apps Script and the Python CLI call this
-- RPC with the service_role key. Validation and writes share one transaction.

alter table public.questions
  add column if not exists content_sync_source text;

-- Adopt rows from the retired external_id sheet pipeline. The first v2 preview
-- will show these old identities as removals before anything destructive runs.
update public.questions
set content_sync_source = 'questions_csv_v2'
where external_id is not null
  and content_sync_source is null;

create index if not exists idx_questions_content_sync_source
  on public.questions (content_sync_source)
  where content_sync_source is not null;
...
begin
  ...
  loop
    ...
    where lower(name) = lower(v_rec.subject_name)
    order by id limit 1;
    if v_subject_id is null then
      insert into public.subjects (name, display_order)
      values (v_rec.subject_name, (select coalesce(max(display_order), 0) + 1 from public.subjects))
  ...
  loop
    ...
    select id into v_subject_id from public.subjects
      where lower(name) = lower(v_rec.subject_name) order by id limit 1;
...
comment on function public.sync_content_csv(jsonb, boolean) is
  'Validates/previews or transactionally applies the one-file Questions CSV snapshot.';
